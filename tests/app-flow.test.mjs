...
test('app flow: saved island, story dialogue, quarterly trigger, menus, intro replay',async()=>{
 ...
 click('story','next');click('story','next');click('story','choose','empire');assert.equal(state().cash,108);click('story','next');assert.equal(state().story.pending,null);
 for(const id of ['research','people','world','council','campus']){click('act','tab',id);assert.ok(app.innerHTML.includes('class="main '+id));}
 click('act','turn');click('act','choice','2');click('act','turn');click('act','turn');assert.equal(state().story.pending.id,'q1');
 for(let i=0;i<3;i++)click('story','next');click('story','choose','accord');click('story','next');
