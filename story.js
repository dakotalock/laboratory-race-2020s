import {modelStoryLine} from './model-system.js';
...
function chapter(s){const q=Math.min(27,Math.max(1,Math.floor(s.month/3))),[title,premise,variants,names]=chapters[q-1],route=storyRoute(s);
 ...
 return {id:`q${q}`,q,title,route,page:0,lines:[premise,variants[routes.indexOf(route)],callback,status,...(s.model?[modelStoryLine(s)]:[])],choices:choices(names,q)};
}
function finale(s){const route=storyRoute(s),kind=s.ending.kind;
 const opening=s.ending.kind.startsWith('model-')?'Julian arrives without a forecast. What happened to LLM-001 has become the story of the world.':{asi:'The threshold is behind us. Julian stands on the dock, holding a folio he has finally stopped pretending contains the whole future.',rival:'The news arrives from another lab. Someone else crossed first. Julian closes the race tracker. The town still needs its ferry.',bankrupt:'The accounts have reached zero. Julian has personally come to help box up the office. For once, the exit strategy involves actual cardboard.',shutdown:'The government has revoked the license. Julian leaves his visitor badge on the table. It is a small object for such a large failure.',time:'The decade ends. No one here crossed the final threshold. Julian has stopped calling every ordinary sunrise a missed milestone.'}[kind];
 ...
 return {id:'finale',title:'The things that remain',route,page:0,lines:[opening,legacy,s.ending.kind.startsWith('model-')?modelStoryLine(s):verdict,'I was going to ask what we do next quarter. Let’s start with tomorrow.'],choices:[]};
}
