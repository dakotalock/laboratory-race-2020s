...
export function buildProfessor(){
 ...
 const bot=new T.Group();bot.position.set(1.35,2.45,.1);group.add(bot);box(mint,0,0,0,.73,.6,.54,bot);box(ink,0,.04,.285,.57,.34,.04,bot);for(const x of [-.16,.16])ell(white,x,.065,.32,.063,.075,.025,bot);tube([[-.1,-.08,.32],[0,-.11,.33],[.1,-.08,.32]],.014,gold,bot);for(const x of [-.47,.47])ell(gold,x,-.06,0,.11,.17,.13,bot);mesh(new T.CylinderGeometry(.045,.045,.2,8),ink,0,.39,0,bot);ell(coral,0,.51,0,.1,.1,.1,bot);
 ...
 const sun=new T.DirectionalLight(0xfff0d8,3);sun.position.set(-3,8,7);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-7,right:7,top:7,bottom:-7,near:.1,far:30});sun.shadow.normalBias=.035;scene.add(sun,new T.HemisphereLight(0xe7fcff,0x81b59e,2.5));const rim=new T.DirectionalLight(0x9fcbff,1.2);rim.position.set(5,4,-2);scene.add(rim);
 let previous=-1,shotTime=0;function update(t,step,aspect){if(previous!==step){previous=step;shotTime=t;}const enter=Math.min(1,(t-shotTime)/1.2);const reveal=step>0;bot.visible=reveal;bot.position.x=aspect<.5?.85:1.2;table.position.x=bot.position.x;bot.position.y=2.2+Math.sin(t*2)*.12;bot.rotation.y=Math.sin(t*.65)*.2;capsules.forEach((c,i)=>{c.rotation.y=t*.35;c.position.y=1.48+Math.sin(t*2+i)*.025;});prof.position.y=Math.sin(t*1.5)*.025;head.rotation.y=Math.sin(t*.7)*.06;head.rotation.z=Math.sin(t*.9)*.025;right.rotation.z=step===0?2.25+Math.sin(t*4.5)*.22:.45+Math.sin(t*1.4)*.09;left.rotation.z=step===1?-.95+Math.sin(t)*.1:-.16;const blink=Math.sin(t*1.8)> .994;head.scale.y=blink?.985:1;camera.aspect=aspect;camera.position.set((step===1?.55:0)+Math.sin(t*.15)*.06,3.05,aspect<.8?10.8:9.4);camera.lookAt(.25,aspect<.8&&step>=4&&step<=8?1.45:2.45,0);camera.updateProjectionMatrix();group.rotation.y=Math.sin(t*.12)*.035;}
